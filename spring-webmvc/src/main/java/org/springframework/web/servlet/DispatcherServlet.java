...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		...
		try {
			...
			// 包括：org.springframework.web.servlet.HandlerMapping 和 org.springframework.web.servlet.HandlerAdapter 等
			// 存放到 defaultStrategies 变量中，后面的相关配置会从 defaultStrategies 读取
	...
	protected void initStrategies(ApplicationContext context) {
		initMultipartResolver(context); // 处理上传文件的 bean，解释了为什么一定要用 multipartResolver，因为写死了
		initLocaleResolver(context); // 国际化的设置
		initThemeResolver(context); // 前端主题样式
		initHandlerMappings(context); // 初始化 HandlerMapping，非常重要，解释了 SpringMVC 默认不支持静态资源的问题
		initHandlerAdapters(context); // 初始化 HandlerAdapter，非常重要
		initHandlerExceptionResolvers(context); // 处理异常的解析
		initRequestToViewNameTranslator(context); // 根据 Request 获取 viewName
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		// 通过配置文件中（DispatcherServlet.properties）的信息得到 handlerMappings
		if (this.handlerMappings == null) {
			// 获取默认 defaultStrategies 中的配置策略
	...
	private void initHandlerAdapters(ApplicationContext context) {
		...
		// 通过配置文件中（DispatcherServlet.properties）的信息得到 handlerAdapters
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		if (this.handlerMappings != null) {
			/*
			handlerMappings 有两个 HandlerMapping 的子类
			1、RequestMappingHandlerMapping
				维护一个：urlLookup 的 Map，里面存放着：映射路径 => method
				处理 @Controller
			2、BeanNameUrlHandlerMapping
				维护一个：handlerMap 的 Map，里面存放着：映射路径 => handler
				处理 BeanName 类型的 Controller
			 */
