 ...
 *
 * 必须设置 parent bean 定义，spring 并没有提供无参构造方法，必须通过构造函数指定
 * 一般使用 ChildBeanDefinition 是确定了父子关系的 BeanDefinition
 *
 * 在 spring2.5 之前据说是配合 RootBeanDefinition 一起使用
 * 但是现在的版本可以配合 {@link GenericBeanDefinition#setParentName} 使用了
 * 未来很可能会被标记过时或者淘汰
 *
 * 这个类的实例化一般是通过构造方法设置的，并且一定要传 父BD 的名字
 * 这是 ChildBeanDefinition 最大的局限了。
 * 所以 ChildBeanDefinition 并不能当作一个普通的 DB 存在
 *
 * @author Rod Johnson
...
public class ChildBeanDefinition extends AbstractBeanDefinition {
	...
	/*
	parentBeanDefinition 的名字
	parentName 是通过构造方法注入的
	这是 ChildBeanDefinition 最大的局限性
	 */
	@Nullable
